...
**Image previews:** Case-study gallery previews carry no scale, lift, border, or scrim on hover (the earlier 1.05x scale + lift was removed 2026-08-05) — hovering rings the frame in a **2px gold outline** and dims the image to `filter: brightness(0.82)`. The outline has `transition: none` — it snaps on and off; only the dim eases, over 0.3s both ways. An earlier version eased the outline out only, by overriding `transition-duration` to `0s` inside the `:hover` rule, which relies on the browser reading transition properties from the after-change style — specified behaviour, but not worth depending on for an effect whose whole point is being immediate. This is the one hover affordance in the system that doesn't flicker: text can blink hard because a dropout reads as a terminal glitch, but on a photograph the same treatment reads as the image failing to load. Two other approaches were built and rejected first — a 3-flash blink, then a 2s sine-wave brightness pulse.

`outline`, not `border` or a ring utility: it's painted outside the border box and takes no part in layout, so the edge appears against the image without nudging it or resizing the frame by a pixel. It starts at `transparent` rather than zero width so there's a value to ease between. And because an outline paints *outside* its element, no ancestor may clip it — the side-by-side variant deliberately moves `overflow-hidden` off the button and onto the inner frame.

Unlike the rest of this section it's **CSS `:hover`, not the JS engine** (which only fires one-shot effects on mouseenter and has no mouseleave path to ease back out of), keyed to `[data-preview]` rather than a bare `.group:hover` since Tailwind's `group` marker is used across the app. The frame carries the outline and the `<img>` carries the dim, which keeps the image-count badge at full brightness and keeps its `backdrop-blur` from being reparented by a `filter` on an ancestor. `:focus-visible` mirrors `:hover` so keyboard users get the same affordance. Each preview is a `<button>` that names opening the image viewer for assistive tech — an appended `sr-only` span where the button already has visible text, an `aria-label` where the image is the only content.
...
Shared layout: back-link to `/work`, company+role eyebrow, case title, date range, stack badges, an optional download button for a case-specific artifact (same treatment as the Resume page's Download PDF), then an optional image gallery section (varies per case, see below), then optional blocky accomplishment callouts for cases that can't show visuals, then long-form narrative body paragraphs, then an optional patent citation (linked + PDF), then prev/next case-study navigation (circular).

The six cases, and what each is *about* (not imagery-heavy vs. text-only is meaningfully different per case):
1. **Action Platform (Discover → Capital One)** — most recent/current role; platform engineering, design-system migration, production data-integrity work during account migrations. No image gallery (NDA, permanently) — carried by four blocky accomplishment callouts instead (`lib/case-study-highlights.ts`).
2. **Aurora Design System (Novant Health)** — translating another company's incomplete visual design into a real system, plus the icon system Derik redefined for it and a physician-finder UX flow. Has a real 5-image gallery, titled "Iconography & Physician Finder" rather than after the system itself, since the component library was never archived: icon system cover, design process, a research board (recognition/info-scent/aesthetics testing), and two wireframe-flow stages. Also the only case study with a downloadable artifact — a 19-page iconography PDF (`lib/case-study-downloads.ts`).
...
4. **Production React/Redux (Agrible → Nutrien)** — engineering-focused case, sustainability/field-data tooling. No images survive from it, so it carries the same four-callout treatment as Action Platform (3 global CPG partners, 4 years of production React/Redux, Weather Story end to end, seasonal report PDFs).
...
Rebuilt 2026-08-05 from option 1b of the "About Body Layouts" Claude Design study ("Banner crop"): a bordered banner pairing the lede and three discipline tags against a wide portrait crop, then the remaining copy set in two newspaper columns with a `column-rule`, then a closing italic line and the contact CTA across a hairline. Replaces a single 68ch prose column with a sticky portrait rail. The study's palette and type were stand-ins — the implementation resolves everything through the real tokens. Copy was tightened by the study to fit the shape (see the file comment for exactly what it drops).
