...
// One-time snapshot of the pre-Contentful hardcoded data (formerly
// lib/case-studies.ts's `caseStudies` export, recovered from git history
// 2026-07-27 when that file was trimmed to just the CaseStudy type). This
...
const caseStudies: CaseStudy[] = [
  {
    ...
    period:
      "Jan 2024 – Present (Discover Jan 2024–Apr 2025; Capital One acquired Discover, continuing May 2025–present)",
    years: "2024–Present",
  ...
  },
  {
    ...
    period:
      "Mar 2015 – Apr 2019 (Agrible Mar 2015–Aug 2018; continued at Nutrien Ag Solutions after acquisition, Aug 2018–Apr 2019)",
    years: "2015–2019",
...
// Contentful's Array field only accepts Symbol/Link items, not Text — long
// multi-paragraph body copy can't be stored as an array. Instead each
// paragraph is joined with a blank line into one Text field and split back
// apart in lib/contentful.ts, so the CaseStudy shape everywhere else is
// unchanged.
async function main() {
  // contentful-management v12's default export is the "Plain Client" (flat
  // resource.method(params, payload) calls), not the older chained
  // space.getEnvironment().createEntry() API most docs/examples still show.
  ...
  let contentType;
  try {
    contentType = await client.contentType.get({ contentTypeId: CONTENT_TYPE_ID });
    console.log("caseStudy content type already exists, reusing.");
  } catch {
    contentType = await client.contentType.createWithId(
      { contentTypeId: CONTENT_TYPE_ID },
      {
        ...
        fields: [
          ...
          { id: "company", name: "Company", type: "Symbol", required: true, localized: false },
          { id: "role", name: "Role", type: "Symbol", required: true, localized: false },
          { id: "period", name: "Period", type: "Symbol", required: true, localized: false },
          { id: "years", name: "Years", type: "Symbol", required: true, localized: false },
          ...
          { id: "hasVisuals", name: "Has Visuals", type: "Boolean", required: true, localized: false },
          { id: "body", name: "Body", type: "Text", required: true, localized: false },
          { id: "patentLabel", name: "Patent Label", type: "Symbol", required: false, localized: false },
          { id: "patentUrl", name: "Patent URL", type: "Symbol", required: false, localized: false },
          { id: "patentPdfPath", name: "Patent PDF Path", type: "Symbol", required: false, localized: false },
        ],
      }
    );
    await client.contentType.publish({ contentTypeId: CONTENT_TYPE_ID }, contentType);
    console.log("Created and published caseStudy content type.");
  }
  ...
  for (const [index, cs] of caseStudies.entries()) {
    const fields: Record<string, { "en-US": unknown }> = {
      slug: { "en-US": cs.slug },
      order: { "en-US": index + 1 },
      company: { "en-US": cs.company },
      role: { "en-US": cs.role },
      period: { "en-US": cs.period },
      years: { "en-US": cs.years },
      title: { "en-US": cs.title },
      summary: { "en-US": cs.summary },
      stack: { "en-US": cs.stack },
      hasVisuals: { "en-US": cs.hasVisuals },
      body: { "en-US": cs.body.join("\n\n") },
    };
    if (cs.patentRef) {
      fields.patentLabel = { "en-US": cs.patentRef.label };
      fields.patentUrl = { "en-US": cs.patentRef.url };
      if (cs.patentRef.pdfPath) {
        fields.patentPdfPath = { "en-US": cs.patentRef.pdfPath };
      }
    }

    let entry;
    try {
      entry = await client.entry.get({ entryId: cs.slug });
      entry.fields = fields;
      entry = await client.entry.update({ entryId: cs.slug }, entry);
      console.log(`Updated entry: ${cs.slug}`);
    } catch {
      entry = await client.entry.createWithId(
        { entryId: cs.slug, contentTypeId: CONTENT_TYPE_ID },
        { fields }
      );
      console.log(`Created entry: ${cs.slug}`);
    }
    await client.entry.publish({ entryId: cs.slug }, entry);
  }
